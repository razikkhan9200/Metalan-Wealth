...
export default function PublicNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/[0.06] bg-[#171717]/70 text-white backdrop-blur-xl">
      <div className="mx-auto flex h-[88px] w-full max-w-[1280px] items-center px-6 lg:px-[80px]">
        <Link
          to={ROUTES.HOME}
        ...
        <nav className="ml-[120px] hidden h-full items-center gap-[39px] lg:flex">
          {navigation.map((item) => (
            <NavLink
              key={item.label}
              to={item.href}
              className="text-[13px] font-medium text-[#8F8F8A] transition-colors duration-200 hover:text-[#F1EEE7]"
            >
              {item.label}
            </NavLink>
          ))}
      ...
      {open && (
        <div className="border-t border-white/[0.06] bg-[#0A0A0A]/95 px-6 py-6 backdrop-blur-xl lg:hidden">
          <nav className="mx-auto flex max-w-7xl flex-col gap-5">
            {navigation.map((item) => (
              <NavLink
                ...
                className="text-[14px] font-medium text-[#8F8F8A] transition-colors hover:text-[#F1EEE7]"
              >
                {item.label}
              </NavLink>
            ))}
