    ...
    <header className="absolute left-0 top-0 z-50 w-full border-b border-white/[0.06] !bg-transparent text-white backdrop-blur-md">
      <div className="mx-auto flex h-[88px] w-full max-w-[1280px] items-center px-6 lg:px-[80px]">
        ...
        <div className="ml-auto hidden items-center gap-[31px] lg:flex">
          ...
          <Link
            to={ROUTES.LOGIN}
            className="whitespace-nowrap text-[14px] font-medium text-[#D4A853] transition-colors duration-200 hover:text-[#F1EEE7]"
          >
            Sign In
          </Link>
          ...
          <nav className="flex flex-col px-6 py-7">
            {navigation.map((item) => (
              <NavLink
                ...
                className="py-[11px] text-[14px] font-medium text-[#8F9A96] transition-colors hover:text-[#F1EEE7]"
              >
                {item.label}
              </NavLink>
            ))}
            ...
            <div className="mt-4 border-t border-white/[0.06] pt-6">
              ...
              <Link
                to={ROUTES.LOGIN}
                onClick={() => setOpen(false)}
                className="mb-5 block text-[14px] font-medium text-[#8F9A96] transition-colors hover:text-[#F1EEE7]"
              >
                Sign In
              </Link>
