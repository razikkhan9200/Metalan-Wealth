  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          role="listbox"
          className="mt-filter-menu absolute left-0 right-0 top-[calc(100%+6px)] z-[110] overflow-hidden rounded-xl border border-white/10 bg-[var(--mt-surface)] p-1 shadow-[0_18px_45px_rgba(0,0,0,0.22)] ring-1 ring-black/10"
        >
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${
                  selected
                    ? "bg-[#d4af6a]/12 text-[#e2c17f]"
                    : "text-slate-300 hover:bg-[#d4af6a]/8 hover:text-white"
                }`}
              >
...
function InvestForm({ faix, funds, navs, initialId, onSubmit }) {
  ...
  const error = !amt
    ...
    : amt > faix
      ...
      : amt < fund.min
        ? `Minimum for this fund is ${fmt(fund.min, 4)} FAIX.`
        : "";
