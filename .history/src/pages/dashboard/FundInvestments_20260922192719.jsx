...
/* Premium gold styling for the investment modal.
 * Keeps the shared UI components but overrides their cyan accents
 * only inside the fund investment form.
 */
const GOLD_MODAL_THEME_CSS = `
  ...
  .mt-gold-modal [class*="bg-white/[0.02]"],
  .mt-gold-modal [class*="bg-white/[0.03]"] {
    transition:
            ...
            {!loading &&
              ...
              filtered.map((f) => {
                ...
                return (
                  <div
                    key={f.id}
                    className={`${CARD} min-w-0 flex flex-col border-white/[0.07] p-4 transition-all duration-300 hover:border-[#d4af6a]/25 hover:shadow-[0_14px_40px_rgba(212,175,106,.07)] sm:p-5`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <span className="grid h-11 w-11 place-items-center rounded-xl bg-[#d4af6a]/10 text-[#d4af6a] ring-1 ring-[#d4af6a]/20">
                    ...
                    {held && (
                      <div className="mt-4 rounded-xl border border-[#d4af6a]/20 bg-[#d4af6a]/[0.05] px-3 py-2 text-[11px] text-[#b18b45]">
                        You hold{" "}
                        {fmt(held.units, 4)} units ·{" "}
                        {fmt(
                          held.units * f.nav,
                          4
                        )}{" "}
                    ...
                    <button
                      ...
                      className={`${BTN.gold} mt-4 w-full shadow-[0_7px_22px_rgba(212,175,106,.10)]`}
                    >
                      <TrendingUp className="h-4 w-4" />
                      Invest
                    </button>
