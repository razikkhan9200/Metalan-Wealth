...
function SelectField({ value, onChange, options, allLabel }) {
  ...
  return (
    <div ref={ref} className="relative w-full sm:w-auto sm:min-w-[150px]">
      <button
        type="button"
      ...
      {open && (
        <div
          role="listbox"
          aria-label={allLabel}
          className="absolute left-0 right-0 top-[calc(100%+6px)] z-[80] overflow-hidden rounded-xl border border-white/10 bg-[#0a1019] p-1.5 shadow-[0_18px_45px_rgba(0,0,0,0.45)] ring-1 ring-black/30"
        >
          {options.map((option) => {
            ...
            return (
              ...
              >
                <span>{label}</span>
                {selected && <span className="ml-3 h-1.5 w-1.5 shrink-0 rounded-full bg-cyan-300" />}
              </button>
            );
          })}
        </div>
  ...
  return (
    <article className="px-3 py-4 sm:px-4">
      <div className="flex min-w-0 items-start gap-3 sm:gap-4">
        <span
          className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ring-1 sm:h-11 sm:w-11 ${ACCENT[tone] || ACCENT.cyan}`}
        >
          {Icon ? <Icon className="h-5 w-5" /> : <ReceiptText className="h-5 w-5" />}
        </span>
  ...
  return (
    <PageShell active="Transactions" onNavigate={onNavigate}>
      <div className="mx-auto w-full max-w-7xl min-w-0 space-y-5 overflow-x-hidden sm:space-y-6">
        ...
        {/* Filters */}
        <section
          className={`${CARD} flex flex-col gap-3 p-3.5 sm:flex-row sm:flex-wrap sm:items-center sm:p-4`}
        >
