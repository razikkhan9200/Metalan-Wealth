 ...
 *                graceful fallback, so a dead link never breaks the layout. Dashboard values are fetched from the authenticated backend API.
 */
...
const FUND_IMAGE_MAP = {
  growth: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?auto=format&fit=crop&w=900&q=85",
  income: "https://images.unsplash.com/photo-1556761175-b413da4baf72?auto=format&fit=crop&w=900&q=85",
  realty: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=900&q=85",
  digital: "https://images.unsplash.com/photo-1620321023374-d1a68fbc720d?auto=format&fit=crop&w=900&q=85",
...
function mapApiFund(fund) {
  ...
  return {
    ...
    img: FUND_IMAGE_MAP[id] || FUND_IMAGE_MAP[fund?.category?.toLowerCase()] || FUND_IMAGE_MAP.growth,
...
function mapApiProperty(property, index) {
  ...
  return {
    ...
    img: property?.propertyImage || property?.images?.find((image) => image?.url)?.url || PROPERTY_IMAGE_FALLBACKS[index % PROPERTY_IMAGE_FALLBACKS.length],
...
function mapApiTransaction(transaction, index) {
  ...
  const meta = Number.isFinite(Number(transaction?.total)) ? Number(transaction.total) : Number(transaction?.amount) || 0;
  const credit = rawType === "Deposit" || rawType === "Dividend";

  return {
    id: String(transaction?._id || transaction?.id || `transaction-${index}`),
    asset: transaction?.asset || "Transaction",
    type,
    amount: credit ? Math.abs(meta) : rawType === "Sell" || rawType === "Withdraw" ? Math.abs(meta) : -Math.abs(meta),
...
function InvestForm({ ctx, initialId, done }) {
  ...
  const error = busy || !amt ? "" : amt > ctx.faix ? "Amount is higher than your available FAIX." : fund.min > 0 && amt < fund.min ? `Minimum for this fund is ${fmt0(fund.min)} FAIX.` : nav <= 0 ? "This fund has no valid NAV yet." : "";
...
function Dashboard({ onNavigateAway = () => {}, initialSection = "Dashboard", lenisRef }) {
  ...
  const [exchange, setExchange] = useState({ value: 0, cost: 0 });
  ...
  const refreshDashboard = useCallback(async () => {
    try {
      ...
      const [dashboardResult, propertiesResult] = await Promise.allSettled([
        get("/dashboard"),
        get("/properties"),
      ]);
      ...
      if (propertiesResult.status === "fulfilled") {
        const allProperties = propertiesResult.value?.data?.properties || [];
        setProperties(allProperties.map(mapApiProperty));
      } else if (data?.topProperties) {
        // Keep the dashboard usable when the dedicated properties endpoint
        // is temporarily unavailable. This still uses API data only.
        setProperties((data.topProperties || []).map(mapApiProperty));
      }
  ...
  const investorPhoto =
    apiWelcome.profileImage ||
    apiWelcome.profilePhoto ||
    apiWelcome.avatarUrl ||
    apiWelcome.avatar ||
    IMAGES.avatar;
  ...
  useEffect(() => {
    ...
    const nextFunds = (dashboardData.topFunds || []).map(mapApiFund);
    const nextProperties = (dashboardData.topProperties || []).map(mapApiProperty);
    ...
    setFunds(nextFunds);
    setProperties((current) => current.length ? current : nextProperties);
    setNavs(Object.fromEntries(nextFunds.map((fund) => [fund.id, fund.nav])));
    setTxs(nextTransactions);
    setUnread(0);
  }, [dashboardData, apiAvailable, apiTotal]);
  ...
  if (dashboardData && anchorRef.current === 0) anchorRef.current = total;
  const anchor = anchorRef.current || total || 1;
  ...
  const actions = {
    ...
    buyProperty: async ({ property, qty, amount }) => {
  ...
  purchasedFaixToken: qty,
    ...
    swap: ({ dir, amount, net }) => {
      if (dir === "in") {
        ...
        setFaix((b) => b + net);
        setExchange((ex) => ({ value: ex.value - amount, cost: ex.value ? ex.cost * ((ex.value - amount) / ex.value) : 0 }));
      }
  ...
  const ctx = { faix, navs, fundPos, funds, properties, propPos, exchange, actions };

  /* ── live market tick ── */
  // NAV values now come from the dashboard API.
  ...
  const topUsers = (dashboardData?.topInvestors?.list || []).slice(0, 5).map((u) => ({
    ...
    profileImage: u.profileImage || u.profilePhoto || u.avatarUrl || u.avatar || null,
  }));
  const topInvestments = [
    ...funds.filter((f) => fundPos[f.id]).map((f) => ({
      id: `fund-${f.id}`, kind: "Fund", name: f.name, sub: f.category, icon: f.icon,
      value: fundPos[f.id].units * (navs[f.id] || f.nav), cost: fundPos[f.id].cost,
    })),
  ...
  const kpis = [
    ...
    { label: "Used amount", value: fmt0(invested), unit: "FAIX", icon: Briefcase, tone: "cyan", note: `${investTxs.length} local activity records` },
              ...
              <div className="flex items-center gap-4">
                <div className="relative h-14 w-14 shrink-0 sm:h-16 sm:w-16">
                  ...
                  <div className="relative grid h-full w-full place-items-center overflow-hidden rounded-full border-2 border-[#05080d] bg-[#1a1710] text-sm font-semibold text-[#e2c17f]">
                    ...
                    <Photo src={investorPhoto} alt={investorName} eager className="absolute inset-0 h-full w-full object-cover" />
                  </div>
      ...
      >
        ...
        <section id="leaders" aria-label="Top performers" className="mx-4 sm:mx-6 lg:mx-8 grid gap-5 lg:gap-6 xl:grid-cols-3">
          ...
          <div data-anim="section" className={`${CARD} p-6`}>
            <SectionTitle icon={Trophy} tone="gold" title="Top 5 properties" subtitle="Ranked by property value"
            ...
            <ul className="space-y-2">
              {topProperties.map((p, i) => {
                const sold = Number.isFinite(Number(p.soldPercent))
                  ...
                  : (p.total > 0 ? pctOf(p.total - p.available, p.total) : 0);
                return (
                  <li key={p.id} className="flex items-center gap-3 rounded-xl px-2.5 py-3 transition-colors hover:bg-white/[0.03]">
                    ...
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${p.status === "Available" ? ACCENT.emerald : ACCENT.gold}`}>{p.status}</span>
                  </li>
                );
              })}
            </ul>
            ...
            <ul className="space-y-2">
              {topUsers.map((u, i) => (
                <li key={u.id} className="flex items-center gap-3 rounded-xl px-2.5 py-3 transition-colors hover:bg-white/[0.03]">
                  <RankBadge rank={i + 1} />
                  <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-full text-xs font-semibold ring-1 ${AVATAR_TONES[i % AVATAR_TONES.length]}`}>{u.initials || initials(u.name)}</span>
                  <div className="min-w-0 flex-1">
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    <p className={`text-[11px] tabular-nums ${u.returns >= 0 ? "text-emerald-300" : "text-rose-300"}`}>+{Math.abs(u.returns).toFixed(2)}% growth</p>
                  </div>
                </li>
              ))}
            </ul>
