    ...
    >
      ...
      {open && (
        <div
          role="listbox"
          aria-label={allLabel}
          className="absolute left-0 right-0 top-[calc(100%+6px)] z-[110] overflow-hidden rounded-xl border border-white/10 bg-[#0a1019] p-1.5 shadow-[0_18px_45px_rgba(0,0,0,0.45)] ring-1 ring-black/30"
        >
          {options.map((option) => {
            ...
            return (
              ...
              >
                <span>{label}</span>
                {selected && (
                  <span className="ml-3 h-1.5 w-1.5 shrink-0 rounded-full bg-cyan-300" />
                )}
              </button>
            );
          })}
