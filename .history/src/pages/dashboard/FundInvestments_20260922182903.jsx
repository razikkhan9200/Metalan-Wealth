  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="flex h-11 w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-[#080d14] px-3.5 text-left text-sm font-medium text-slate-200 outline-none transition-colors hover:border-white/15 focus:border-cyan-400/50 focus:ring-1 focus:ring-cyan-400/20"
      >
      ...
      {open && (
        <div
          role="listbox"
          className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-xl border border-white/10 bg-[#0a1019] p-1 shadow-2xl shadow-black/40"
        >
          {options.map((option) => {
...
export default function FundInvestments({ onNavigate = () => {} }) {
  ...
  return (
    <PageShell active="Investments" onNavigate={onNavigate}>
      ...
      {/* stats */}
      <section aria-label="Fund portfolio overview" className="grid grid-cols-1 gap-3 min-[430px]:grid-cols-2 lg:grid-cols-4">
      ...
      {/* filters */}
      <section className={`${CARD} flex flex-col gap-3 p-3.5 sm:flex-row sm:flex-wrap sm:items-center sm:p-4`}>
      ...
      <section aria-label="Available funds">
        ...
        <div className="grid gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-3">
          {!loading && !loadError && filtered.map((f) => {
            ...
            return (
              <div key={f.id} className={`${CARD} min-w-0 flex flex-col p-4 transition-colors hover:border-white/15 sm:p-5`}>
                ...
                {held && (
                  <div className="mt-4 rounded-xl border border-cyan-400/20 bg-cyan-400/[0.05] px-3 py-2 text-[11px] text-cyan-200">
                    You hold {fmt(held.units, 4)} units · {fmt(held.units * f.nav, 4)} FAIX
                  </div>
                )}
...
function InvestForm({ faix, funds, navs, initialId, onSubmit }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <AmountField label="Investment amount" unit="FAIX" value={amount} onChange={setAmount} hint={`Min ${fmt(fund.min, 4)}`}
