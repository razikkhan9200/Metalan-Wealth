...
// const METHODS = [
//   { id: "Bank transfer", icon: Landmark, comingSoon: true },
//   { id: "Card", icon: CreditCard, comingSoon: true },
//   { id: "Crypto wallet", icon: WalletIcon, comingSoon: true },
//   { id: "Admin", icon: ShieldCheck, comingSoon: false },
// ];
...
//   const [exchange] = useState({ value: 0, cost: 0 });
//   const [txs, setTxs] = useState([]);
//   const [modal, setModal] = useState(null); // "deposit" | "withdraw" | "exchange" | "admin" | null
//   const [toast, setToast] = useState(null);
//   const [copied, setCopied] = useState(false);
//   const [loading, setLoading] = useState(true);
//   const [loadError, setLoadError] = useState("");
...
//             <DepositForm
//               faix={faix}
//               onSubmit={(p) => { addFunds(p); close(); }}
//               onComingSoon={(methodName) => notify(`${methodName} is coming soon`)}
//               onDone={close}
//             />
//           )}
//         </Modal>
...
// function DepositForm({ faix, onSubmit, onComingSoon, onDone }) {
...
//           Admin tak aapka message pahunch gaya hai. Ek ghante ke andar admin aap se coordination karega.
//         </p>
//         <button type="button" onClick={onDone} className={`${BTN.gold} px-6`}>
//           Okay
//         </button>
//       </div>
//     );
...
//         {METHODS.map(({ id, icon: Icon, comingSoon }) => (
//           <button key={id} type="button" onClick={() => (comingSoon ? onComingSoon(id) : setMethod(id))}
//             className={`flex flex-col items-center gap-1.5 rounded-xl border px-2 py-3 text-[11px] transition-colors ${method === id ? "border-[#d4af6a]/45 bg-[#d4af6a]/[0.08] text-white ring-1 ring-[#d4af6a]/15" : "border-white/10 bg-white/[0.02] text-slate-400 hover:border-white/20"}`}>
//             <Icon className="h-4 w-4 shrink-0" /><span className="truncate">{id}</span>
//             {comingSoon && <span className="rounded-full bg-[#d4af6a]/15 px-1.5 py-0.5 text-[8px] font-semibold uppercase tracking-wide text-[#e2c17f]">Soon</span>}
//           </button>
//         ))}
//       </div>
//       <AmountField label="Amount to add" unit="FAIX" value={amount} onChange={setAmount}
...
//       <button
//         type="button"
//         disabled={amt <= 0 || !!error}
//         onClick={() => { if (method === "Admin") setSent(true); else onSubmit({ amount: amt, method }); }}
  ...
  if (sent) {
    return (
      <div className="flex flex-col items-center gap-4 py-8 text-center">
        ...
        <p className="max-w-xs text-sm text-slate-200">
         Your request has reached the admin. They'll get in touch with you within an hour to coordinate.
        </p>
        <button type="button" onClick={onDone} className={`${BTN.gold} px-6`}>
          Okay
        </button>
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {METHODS.map(({ id, icon: Icon, comingSoon }) => (
          <button key={id} type="button" onClick={() => (comingSoon ? onComingSoon(id) : setMethod(id))}
            className={`flex min-h-[84px] flex-col items-center justify-center gap-1 rounded-xl border px-2 py-3 text-center text-[11px] leading-tight transition-colors ${method === id ? "border-[#d4af6a]/45 bg-[#d4af6a]/[0.08] text-white ring-1 ring-[#d4af6a]/15" : "border-white/10 bg-white/[0.02] text-slate-400 hover:border-white/20"}`}>
            <Icon className="h-4 w-4 shrink-0" />
            <span className="break-words">{id}</span>
            <span className={`rounded-full px-1.5 py-0.5 text-[8px] font-semibold uppercase tracking-wide ${comingSoon ? "bg-[#d4af6a]/15 text-[#e2c17f]" : "invisible"}`}>
              Soon
            </span>
          </button>
        ))}
