...
function Dashboard({ onNavigateAway = () => {}, initialSection = "Dashboard", lenisRef }) {
  ...
  const refreshDashboard = useCallback(async () => {
    try {
      ...
      const mappedProperties = allProperties.map(mapApiProperty);
      setFunds(apiFunds.map(mapApiFund));
      setProperties(mappedProperties);

      const transactionRecords = Array.isArray(data?.transactionHistory?.records)
        ? data.transactionHistory.records
        : [];

      // Property holdings are reconstructed from API purchase activity because
      // this dashboard currently has no separate property-holdings endpoint.
      const nextPropertyPositions = {};
      transactionRecords.forEach((item) => {
        const transactionType = String(item?.type || "").trim();
        if (transactionType !== "Buy" && transactionType !== "Sell") return;

        const propertyName = item?.propertyName || item?.asset || item?.name;
        if (!propertyName) return;

        const property = mappedProperties.find((p) => p.name === propertyName);
        if (!property) return;

        const explicitTokens = Number(
          item?.purchasedFaixToken ??
          item?.purchasedTokens ??
          item?.faixTokenUsed ??
          item?.tokens ??
          item?.quantity
        );
        const cost = Math.abs(Number(item?.total ?? item?.amount ?? item?.purchaseAmount) || 0);
        const derivedTokens = explicitTokens > 0
          ? explicitTokens
          : property.tokenPrice > 0
            ? cost / property.tokenPrice
            : 0;

        if (!Number.isFinite(derivedTokens) || derivedTokens <= 0) return;

        const sign = transactionType === "Sell" ? -1 : 1;
        const current = nextPropertyPositions[property.id] || { tokens: 0, cost: 0 };
        nextPropertyPositions[property.id] = {
          tokens: Math.max(0, current.tokens + sign * derivedTokens),
          cost: Math.max(0, current.cost + sign * cost),
        };
      });
  ...
  const apiPropertyInvestments = txs
    .filter((t) => t.type === "Buy")
    .map((t) => {
      const property = properties.find((p) => p.name === t.asset);
      if (!property) return null;
      const cost = Math.abs(Number(t.amount) || 0);
      const tokens = property.tokenPrice > 0 ? cost / property.tokenPrice : 0;
      return {
        id: `prop-tx-${t.id}`,
        kind: "Property",
        name: property.name,
        ...
        icon: Building2,
        value: tokens * property.tokenPrice,
        cost,
      };
    })
    .filter(Boolean);
  ...
  const kpis = [
    ...
    { label: "Used amount", value: fmt(invested), unit: "FAIX", icon: Briefcase, tone: "cyan", note: `${investTxs.length} API activity records` },
  ...
  id="dashboard"
  ref={root}
  className={`min-h-screen w-full min-w-0 bg-[#05080d] text-slate-200 antialiased
    transition-[margin] duration-300 ml-[76px]
    ${sidebarExpanded ? "md:ml-[250px]" : "md:ml-[76px]"}`}
  style={{ backgroundImage: BG }}
>
      <style>{STYLES}</style>
      <div className="mx-auto w-full max-w-[1440px] min-w-0 space-y-6 px-3 pb-6 pt-[88px] sm:px-5 md:py-6 lg:px-8">
        {/* ───────── hero ───────── */}
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <h1 className="truncate text-xl font-semibold text-white sm:text-2xl">{investorName}</h1>
  ...
  {notifOpen && (
    <div className="absolute right-0 z-50 mt-3 w-[340px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-white/10 bg-[#0a1019] shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
      ...
      <div className="border-t border-white/[0.06] px-4 py-2.5 text-center">
        ...
        {/* ───────── quick actions ───────── */}
        <section id="exchange" aria-label="Quick actions" className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {QUICK_ACTIONS.map((a) => (
          ...
          <section data-anim="chart" className={`${CARD} min-w-0 p-4 sm:p-6 xl:col-span-2`}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex w-full flex-wrap items-center gap-2 lg:w-auto lg:justify-end">
                <div className="flex min-w-0 max-w-full items-center rounded-xl border border-white/10 bg-white/[0.02]">
                  ...
                  <span className="flex min-w-0 max-w-[10.5rem] items-center justify-center gap-2 px-1 text-[11px] text-slate-300 sm:min-w-[9.5rem] sm:text-xs">
                ...
                <div role="group" aria-label="Time filter" className="flex min-w-0 max-w-full overflow-x-auto scrollbar-hide rounded-xl border border-white/10 bg-white/[0.02] p-1">
                  {Object.entries(RANGES).map(([key, r]) => (
                    <button key={key} type="button" aria-pressed={range === key} onClick={() => { setRange(key); setOffset(0); }}
                      className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[11px] font-medium transition-colors sm:px-3 sm:text-xs ${range === key ? "bg-cyan-400/15 text-cyan-200" : "text-slate-400 hover:text-white"}`}>
                      {r.label}
                    </button>
                  ))}
                </div>
            ...
            <div ref={chartWrap} className="mt-3 h-[240px] w-full min-w-0 sm:h-[300px] md:h-[340px] lg:h-[380px] [&_*]:outline-none">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart key={`${range}-${offset}`} data={series.data} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fill: "#64748b", fontSize: 10 }} minTickGap={24} dy={8} />
                  <YAxis domain={[yMin, yMax]} width={44} tickLine={false} axisLine={false} tick={{ fill: "#64748b", fontSize: 10 }}
                    tickFormatter={(v) => new Intl.NumberFormat("en-US", { notation: "compact", maximumFractionDigits: 4 }).format(v)} />
  ...
  {kpis.map((k) => (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="grid min-w-0 gap-5 md:grid-cols-2 2xl:grid-cols-3">
            ...
            <div data-anim="section" className={`${CARD} overflow-hidden`}>
              ...
              <div className="divide-y divide-white/[0.05]">
                {topProperties.map((p, i) => (
                  <div key={p.id} className="flex min-w-0 items-center gap-3 px-4 py-3.5 transition-colors hover:bg-white/[0.03] sm:px-5">
                    <RankBadge rank={i + 1} />
            ...
            <div data-anim="section" className={`${CARD} overflow-hidden`}>
              ...
              <div className="divide-y divide-white/[0.05]">
                {topUsers.map((u, i) => (
                  <div key={u.id} className="flex min-w-0 items-center gap-3 px-4 py-3.5 transition-colors hover:bg-white/[0.03] sm:px-5">
                    <RankBadge rank={i + 1} />
              ...
              {topInvestments.length === 0 ? (
                ...
                <div className="divide-y divide-white/[0.05]">
                  {topInvestments.map((inv, i) => (
                    <div key={inv.id} className="flex min-w-0 items-center gap-3 px-4 py-3.5 transition-colors hover:bg-white/[0.03] sm:px-5">
                      <RankBadge rank={i + 1} />
        ...
        <section id="investments" data-anim="section" className="mx-0 my-6 lg:my-8">
  ...
  return (
    <article
      key={f.id}
      className="
        group relative w-[min(245px,calc(100vw-7rem))] shrink-0 overflow-hidden
    ...
    >
      ...
      <div className="p-4">
        ...
        <section id="properties" data-anim="section" className="mx-0">
          ...
          <div ref={propsCar.ref} {...propsCar.handlers} className={`${CAROUSEL} mt-1`}>
            {properties.map((p) => {
              ...
              return (
                <article key={p.id} data-hover="lift" className={`${CARD} group flex w-[min(280px,calc(100vw-7rem))] shrink-0 snap-start flex-col overflow-hidden transition-colors hover:border-[#d4af6a]/25 sm:w-[310px]`}>
        ...
        {/* ───────── recent transactions ───────── */}
        <section id="transactions" data-anim="section" className={`${CARD} mx-0 mb-8 overflow-hidden`}>
