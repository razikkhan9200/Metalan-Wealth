...
const CAROUSEL =
  "mt-hide-scroll flex select-none snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain px-1 pb-4 pt-2";
...
function useCarousel() {
  ...
  const handlers = {
    ...
    onPointerMove: (e) => {
      ...
      el.scrollLeft = d.startLeft - distance;
    },
...
>
      <style>{STYLES}</style>
      <div className="mx-auto min-w-0 max-w-[1440px] space-y-6 px-4 pb-6 pt-[88px] sm:px-6 md:py-6 lg:px-8">
        {/* ───────── hero ───────── */}
        ...
        {/* ───────── quick actions ───────── */}
        <section id="exchange" aria-label="Quick actions" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {QUICK_ACTIONS.map((a) => (
        ...
        <div className="grid min-w-0 gap-5 2xl:grid-cols-3">
          <section data-anim="chart" className={`${CARD} min-w-0 p-4 sm:p-6 2xl:col-span-2`}>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 border-t border-white/[0.06] pt-4 sm:grid-cols-2 xl:grid-cols-4">
              {kpis.map((k) => (
                <div
                  key={k.label}
                  className="group flex min-w-0 items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-[#0b121c]/70 px-3.5 py-3 transition-all duration-200 hover:border-white/[0.12] hover:bg-white/[0.025]"
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-1.5 flex min-w-0 items-baseline gap-1.5">
                      <span className="truncate text-lg font-semibold leading-none tracking-tight text-white">
                        {k.value}
                      </span>
                      {k.unit && (
                  ...
                  <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ring-1 ${ACCENT[k.tone]}`}>
          ...
          <div className="grid min-w-0 gap-5 sm:grid-cols-2 2xl:grid-cols-1">
            {/* asset allocation */}
            <section id="portfolio" data-anim="section" className={`${CARD} min-w-0 p-4 sm:p-5`}>
            ...
            {/* portfolio summary */}
            <section id="wallet" data-anim="section" className={`${CARD} min-w-0 p-4 sm:p-5`}>
        ...
        <section id="investments" data-anim="section">
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <section id="properties" data-anim="section">
        ...
        {/* ───────── recent transactions ───────── */}
        <section id="transactions" data-anim="section" className={CARD}>
