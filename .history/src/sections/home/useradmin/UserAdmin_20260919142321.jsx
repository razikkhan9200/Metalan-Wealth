...
const BG =
  "radial-gradient(65rem 30rem at 88% -8%, rgba(34,211,238,0.045), transparent 60%), radial-gradient(55rem 30rem at -8% 4%, rgba(212,175,106,0.055), transparent 60%), radial-gradient(70rem 36rem at 50% 100%, rgba(10,55,42,0.18), transparent 65%)";
...
const FUNDS = [
  ...
  },
  {
    ...
    min: 100,
    icon: Coins,
    image: "https://images.unsplash.com/photo-1620321023374-d1a68fbc720d?auto=format&fit=crop&w=900&q=80",
  },
];
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[92vh] w-full overflow-y-auto overscroll-contain rounded-t-3xl border border-white/10 bg-[#0a1019] p-6 shadow-2xl sm:max-w-md sm:rounded-3xl"
    ...
    <div className="space-y-5">
      <OptionList value={id} onChange={setId}
        options={ctx.properties.map((p) => ({
          ...
          title: p.name,
          sub: p.location,
          meta: `${fmt0(p.tokenPrice)} FAIX / token`,
          image: p.image,
        }))} />
  ...
  return (
    <div ref={root} className="min-h-screen bg-[#05080d] text-slate-200 antialiased" style={{ backgroundImage: BG }}>
      <div className="mx-auto max-w-[1440px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        {/* ───────── header ───────── */}
        <header data-anim="header" className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="grid h-12 w-12 place-items-center rounded-full bg-[#d4af6a]/15 text-sm font-semibold text-[#e2c17f] ring-1 ring-[#d4af6a]/40">AM</div>
            <div>
              <p className="text-xs text-slate-500">{greeting}</p>
              <h1 className="text-xl font-semibold text-white sm:text-2xl">Andrew Doe</h1>
            </div>
          </div>

          <div className="order-last flex w-full items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-2 md:order-none md:w-auto">
            <ShieldCheck className={`h-4 w-4 ${monthGrowth >= 0 ? "text-emerald-300" : "text-rose-300"}`} />
            <div>
              <p className="text-[11px] text-slate-500">Portfolio status</p>
              <p className="text-sm font-medium text-white">
                {monthGrowth >= 0 ? "Performing well" : "Below trend"}{" "}
                <span className={monthGrowth >= 0 ? "text-emerald-300" : "text-rose-300"}>{signedPct(monthGrowth)} in 30 days</span>
              </p>
            </div>
          </div>
        ...
        <section data-anim="section">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {FUNDS.map((f) => {
              ...
              return (
                <article key={f.id} data-hover="lift" className={`${CARD} group flex flex-col p-5 transition-colors hover:border-cyan-400/25`}>
                  <div className="relative -mx-5 -mt-5 h-32 overflow-hidden rounded-t-2xl border-b border-white/5">
                    <img
                      src={f.image}
                      alt={f.name}
                    ...
                    <span className={`absolute left-4 top-4 grid h-10 w-10 place-items-center rounded-xl ring-1 backdrop-blur-md ${ACCENT.cyan}`}>
                      <f.icon className="h-5 w-5" />
                    </span>
                    <span className={`absolute right-4 top-4 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset backdrop-blur-md ${RISK[f.risk]}`}>
                      {f.risk} risk
                    </span>
                  </div>
                  <h3 className="mt-4 text-[15px] font-semibold text-white">{f.name}</h3>
            ...
            {properties.map((p) => {
              ...
              return (
                <article key={p.id} data-hover="lift" className={`${CARD} flex flex-col overflow-hidden transition-colors hover:border-[#d4af6a]/25`}>
                  <div className="relative h-36 overflow-hidden border-b border-white/5 bg-[#0d1520]">
                    <img
                      src={p.image}
                      alt={p.name}
                      ...
                      loading="lazy"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0a1019] via-transparent to-black/20" />
                    <span className="absolute left-4 top-4 inline-flex items-center gap-1 rounded-full bg-black/35 px-2 py-1 text-[11px] text-slate-200 backdrop-blur-md">
                      <MapPin className="h-3 w-3" />{p.location}
                    </span>
                    <span className={`absolute right-4 top-4 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset backdrop-blur-md ${ACCENT.emerald}`}>
                      {p.yield}% yield
                    </span>
                  </div>
