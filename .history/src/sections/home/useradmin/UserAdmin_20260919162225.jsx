...
>
      ...
      <div className="mx-auto max-w-[1440px] space-y-6 px-4 pb-6 pt-[88px] sm:px-6 md:py-6 lg:px-8">
        ...
        <div className="grid gap-6 xl:grid-cols-3">
         ...
         <div className="mt-4 grid grid-cols-1 gap-0 border-t border-white/[0.06] pt-4 sm:grid-cols-2 lg:grid-cols-4">
  {kpis.map((k, index) => (
    <div
      key={k.label}
      className={`group flex min-w-0 items-center gap-3 px-0 py-3 sm:px-4 lg:py-2.5 ${
        index !== 0
          ? "border-white/[0.06] sm:border-l"
          : ""
      }`}
    >
      {/* Icon */}
      <div
        className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ring-1 transition-all duration-200 group-hover:scale-[1.03] ${ACCENT[k.tone]}`}
      >
        <k.icon className="h-[17px] w-[17px]" />
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 truncate text-[17px] font-semibold leading-none tracking-tight text-white">
          {k.value}
          {k.unit && (
