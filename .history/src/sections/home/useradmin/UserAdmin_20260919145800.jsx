 ...
 *                graceful fallback, so a dead link never breaks the layout. Swap URLs in IMAGES / FUNDS / PROPERTIES.
 */
...
/**
 * SIDEBAR LAYOUT
 * NavigationPanel desktop par fixed sidebar hai, isliye page content ko uski width ke barabar left offset chahiye.
 * `lg:pl-72` = 18rem (288px). Agar tumhare sidebar ki width alag hai (jaise 260px / 80px collapsed),
 * to sirf ye ek line badlo, e.g. "lg:pl-64" (256px) ya "lg:pl-[260px]".
 * Mobile par sidebar top-bar / drawer hota hai, isliye `pt-16` top bar ki jagah chhodta hai (jarurat na ho to hata do).
 */
const NAV_OFFSET = "pt-16 lg:pt-0 lg:pl-72";
...
// Mouse: drag ya wheel/shift+wheel. Touch: ungli se left-right swipe, aur upar-neeche page scroll bhi chalta rahega.
const CAROUSEL =
  "mt-hide-scroll -mx-4 flex select-none snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain px-4 pb-4 pt-2 scroll-pl-4 sm:-mx-1 sm:px-1 sm:scroll-pl-1 [touch-action:pan-x_pan-y] [-webkit-overflow-scrolling:touch]";

const STYLES = `
.mt-hide-scroll{scrollbar-width:none;-ms-overflow-style:none}
...
const FUNDS = [
  { id: "growth", name: "Metalan Growth Fund", category: "Equity growth", nav: 12.485, growth: 18.4, risk: "Moderate", status: "Open", min: 100, icon: TrendingUp, img: U("photo-1611974789855-9c2a0a7236a3") },
  { id: "income", name: "Sovereign Income Fund", category: "Fixed income", nav: 10.213, growth: 7.2, risk: "Low", status: "Open", min: 100, icon: ShieldCheck, img: U("photo-1554224155-6726b3ff858f") },
  { id: "realty", name: "Prime Realty REIT Fund", category: "Real estate", nav: 24.76, growth: 11.6, risk: "Moderate", status: "Limited", min: 250, icon: Building2, img: U("photo-1560518883-ce09059eeffa") },
  { id: "digital", name: "Digital Assets Alpha", category: "Digital assets", nav: 8.942, growth: 26.9, risk: "High", status: "Open", min: 100, icon: Coins, img: U("photo-1518546305927-5a555bb7020d") },
  { id: "tech", name: "Global Tech Leaders Fund", category: "Technology equity", nav: 15.372, growth: 21.3, risk: "High", status: "Open", min: 100, icon: Cpu, img: U("photo-1518770660439-4636190af475") },
  { id: "green", name: "Green Energy Transition", category: "Sustainable energy", nav: 9.318, growth: 14.1, risk: "Moderate", status: "Open", min: 150, icon: Leaf, img: U("photo-1473341304170-971dccb5ac1e") },
];
...
function Toast({ message }) {
  ...
  return (
    <div ref={ref} className="fixed bottom-6 right-4 z-[80] flex max-w-[calc(100vw-2rem)] items-center gap-3 rounded-xl border border-emerald-400/20 bg-[#0a1019] px-4 py-3 text-sm text-slate-100 shadow-2xl sm:right-6">
...
/** Tiny seeded trend line for a fund card (dummy data, ends higher for higher 1-year growth). */
function Spark({ id, seed, growth }) {
  const { line, area } = useMemo(() => {
    const r = rng(seed * 131 + 7);
    const n = 24;
    const raw = Array.from({ length: n }, (_, i) => {
      const t = i / (n - 1);
      return t * (growth / 100) + (r() - 0.5) * 0.05 * Math.sin(Math.PI * t);
    });
    const lo = Math.min(...raw);
    const hi = Math.max(...raw);
    const pts = raw.map((v, i) => [(i / (n - 1)) * 100, 28 - ((v - lo) / (hi - lo || 1)) * 24]);
    const path = pts.map(([x, y], i) => `${i ? "L" : "M"}${x.toFixed(1)} ${y.toFixed(1)}`).join(" ");
    return { line: path, area: `${path} L100 32 L0 32 Z` };
  }, [seed, growth]);
  return (
    <svg viewBox="0 0 100 32" preserveAspectRatio="none" className="h-9 w-full" aria-hidden>
      ...
      <path d={line} fill="none" stroke={C.cyan} strokeWidth="1.5" vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" />
    </svg>
  );
}

/** One fund in the carousel: photo header, live NAV, trend line, and the user's own position. */
function FundCard({ fund: f, nav, pos, index, onInvest }) {
  const value = pos ? pos.units * nav : 0;
  const pnl = pos && pos.cost ? (value / pos.cost - 1) * 100 : null;
  return (
    <article data-hover="lift" className={`${CARD} group flex w-[280px] shrink-0 snap-start flex-col overflow-hidden transition-colors hover:border-cyan-400/25 sm:w-[310px]`}>
      <div className="relative h-36 overflow-hidden bg-gradient-to-br from-[#0f2230] to-[#0a1019]">
        <Photo src={f.img} alt={f.name} className="absolute inset-0 h-full w-full object-cover group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a1019] via-[#0a1019]/25 to-black/40" />
        <span className={`absolute left-3 top-3 grid h-10 w-10 place-items-center rounded-xl bg-black/50 ring-1 backdrop-blur ${ACCENT.cyan}`}>
          <f.icon className="h-5 w-5" />
        </span>
        <span className="absolute right-3 top-3 rounded-full bg-black/55 backdrop-blur">
          <span className={`block rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${RISK[f.risk]}`}>{f.risk} risk</span>
        </span>
        <span className="absolute bottom-3 left-3 rounded-full bg-black/45 px-2.5 py-1 text-[11px] text-slate-100 backdrop-blur">{f.category}</span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-3 flex items-end justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold tabular-nums text-white">{fmt(nav, 4)}</p>
          </div>
          <div className="flex flex-col items-end gap-1">
            <Delta value={f.growth} />
            <span className="text-[11px] text-slate-500">1-year growth</span>
          </div>
        </div>

        <div className="mt-2"><Spark id={f.id} seed={index + 1} growth={f.growth} /></div>

        <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-3 border-t border-white/5 pt-4 text-xs">
          <div>
            <dt className="text-slate-500">Minimum</dt>
            <dd className="mt-0.5 text-sm font-medium tabular-nums text-white">{fmt0(f.min)} FAIX</dd>
          </div>
          <div className="text-right">
            <dt className="text-slate-500">Status</dt>
            <dd className="mt-0.5 inline-flex items-center gap-1.5 text-sm font-medium text-white">
              <span className={`h-1.5 w-1.5 rounded-full ${f.status === "Open" ? "bg-emerald-400" : "bg-amber-300"}`} />{f.status}
            </dd>
          </div>
          <div>
            <dt className="text-slate-500">Your units</dt>
            <dd className="mt-0.5 text-sm font-medium tabular-nums text-white">{pos ? fmt(pos.units, 2) : "None yet"}</dd>
          </div>
          <div className="text-right">
            <dt className="text-slate-500">Your value</dt>
            <dd className="mt-0.5 text-sm font-medium tabular-nums text-cyan-300">
              {pos ? `${fmt0(value)} FAIX` : "-"}
              {pnl != null && <span className={`ml-1 text-[11px] ${pnl >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{signedPct(pnl)}</span>}
            </dd>
          </div>
        </dl>

        <button type="button" data-hover="btn" onClick={onInvest} className={`${BTN.cyanSoft} mt-5`}>
...
function Modal({ title, subtitle, icon: Icon, tone, onClose, children }) {
  ...
  return (
    ...
    >
      {/* Scrollbar hidden: mouse wheel aur ungli (touch) se scroll hota hai. */}
      <div ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className="mt-hide-scroll max-h-[92vh] w-full overflow-y-auto overscroll-contain rounded-t-3xl border border-white/10 bg-[#0a1019] p-6 shadow-2xl outline-none [touch-action:pan-y] [-webkit-overflow-scrolling:touch] sm:max-w-md sm:rounded-3xl">
 ...
 * Scrollbar poori tarah hidden hai. Mouse wheel se scroll hota hai, aur touch par ungli se drag karne se
 * (touch-action: pan-y) list smoothly move karti hai.
 */
function OptionList({ options, value, onChange }) {
  ...
  return (
    <div ref={listRef} className="mt-hide-scroll relative grid max-h-64 gap-2 overflow-y-auto overscroll-contain scroll-smooth [touch-action:pan-y] [-webkit-overflow-scrolling:touch]">
      {options.map((o) => (
...
function Dashboard() {
  ...
  return (
    <>
      ...
      />
      {/* NAV_OFFSET: sidebar ke neeche content na chhupe, isliye desktop par left padding + mobile par top-bar ki jagah. */}
      <div ref={root} className={`min-h-screen bg-[#05080d] text-slate-200 antialiased ${NAV_OFFSET}`} style={{ backgroundImage: BG }}>
        <style>{STYLES}</style>
        <div className="mx-auto max-w-[1440px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <section data-anim="section">
            ...
            <div ref={fundsCar.ref} {...fundsCar.handlers} className={CAROUSEL}>
              {FUNDS.map((f, i) => (
                <FundCard key={f.id} fund={f} index={i} nav={navs[f.id]} pos={fundPos[f.id]} onInvest={() => setModal({ type: "invest", id: f.id })} />
              ))}
            </div>
          </section>
