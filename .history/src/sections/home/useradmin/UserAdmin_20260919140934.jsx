...
const PROPERTIES = [
  { id: "marina", name: "Azure Marina Residences", location: "Dubai Marina, UAE", value: 4850000, tokenPrice: 250, total: 19400, available: 7420, yield: 6.2 },
  { id: "bkc", name: "Central Park Offices", location: "Bandra Kurla, Mumbai", value: 3200000, tokenPrice: 200, total: 16000, available: 5180, yield: 7.4 },
  { id: "aldwych", name: "The Aldwych Suites", location: "Covent Garden, London", value: 6400000, tokenPrice: 500, total: 12800, available: 3960, yield: 4.9 },
  { id: "palm", name: "Palm Grove Villas", location: "Phuket, Thailand", value: 1950000, tokenPrice: 150, total: 13000, available: 4310, yield: 8.1 },
];
        ...
        <header data-anim="header" className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-semibold text-white sm:text-2xl">Alex Morgan</h1>
            </div>
          </div>
        ...
        <section data-anim="section">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {FUNDS.map((f) => {
              ...
              return (
                <article key={f.id} data-hover="lift" className={`${CARD} flex flex-col p-5 transition-colors hover:border-cyan-400/25`}>
                  <div className="flex items-start justify-between gap-3">
                    <span className={`grid h-11 w-11 place-items-center rounded-xl ring-1 ${ACCENT.cyan}`}><f.icon className="h-5 w-5" /></span>
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${RISK[f.risk]}`}>{f.risk} risk</span>
                  </div>
            ...
            {properties.map((p) => {
              ...
              return (
                <article key={p.id} data-hover="lift" className={`${CARD} flex flex-col overflow-hidden transition-colors hover:border-[#d4af6a]/25`}>
                  <div className="relative flex h-28 items-center justify-center border-b border-white/5 bg-[#0d1520]">
                    <Building2 className="h-10 w-10 text-[#d4af6a]/70" strokeWidth={1.25} />
                    <span className="absolute left-4 top-4 inline-flex items-center gap-1 text-[11px] text-slate-400"><MapPin className="h-3 w-3" />{p.location}</span>
                    <span className={`absolute right-4 top-4 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${ACCENT.emerald}`}>{p.yield}% yield</span>
                  </div>
