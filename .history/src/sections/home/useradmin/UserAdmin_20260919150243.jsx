...
const CAROUSEL =
  "mt-hide-scroll flex select-none snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain px-1 pb-4 pt-2 touch-pan-x";
...
const FUNDS = [
  ...
  { id: "income", name: "Sovereign Income Fund", category: "Fixed income", nav: 10.213, growth: 7.2, risk: "Low", status: "Open", min: 100, icon: ShieldCheck, img: U("photo-1556761175-b413da4baf72") },
  { id: "realty", name: "Prime Realty REIT Fund", category: "Real estate", nav: 24.76, growth: 11.6, risk: "Moderate", status: "Limited", min: 250, icon: Building2, img: U("photo-1486406146926-c627a92ad1ab") },
  { id: "digital", name: "Digital Assets Alpha", category: "Digital assets", nav: 8.942, growth: 26.9, risk: "High", status: "Open", min: 100, icon: Coins, img: U("photo-1639762681057-408e52192e55") },
...
function useCarousel() {
  ...
  const scrollByPage = useCallback((dir) => {
    ...
    el.scrollBy({
      left: dir * Math.max(el.clientWidth * 0.78, 260),
      behavior: "smooth",
    });
  }, []);
  ...
  const handlers = {
    ...
    onPointerLeave: (e) => {
      if (e.pointerType === "mouse" && drag.current.active) finish(e);
    },
    ...
    onWheel: (e) => {
      ...
      // Trackpad naturally scrolls horizontally. For a normal mouse wheel,
      // convert vertical wheel movement into horizontal card scrolling.
      if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        const canMove =
          (e.deltaY > 0 && !edge.end) ||
          (e.deltaY < 0 && !edge.start);
  ...
  return (
    ...
    <div ref={root} className="min-h-screen bg-[#05080d] text-slate-200 antialiased" style={{ backgroundImage: BG }}>
      ...
      <div className="mx-auto max-w-[1440px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        {/* ───────── fund investments (advanced draggable cards) ───────── */}
        <section data-anim="section">
          <SectionTitle
            icon={Layers}
            tone="cyan"
            title="Fund investments"
            subtitle="Live NAV, holdings and growth across your investment funds."
            right={
              <div className="flex items-center gap-2">
                <span className="hidden items-center gap-1.5 rounded-full bg-white/[0.04] px-2.5 py-1 text-[11px] text-slate-400 md:inline-flex">
          ...
          <div
            ref={fundsCar.ref}
            {...fundsCar.handlers}
            className={`${CAROUSEL} -mx-1`}
            style={{ touchAction: "pan-x" }}
          >
            {FUNDS.map((f) => {
              const pos = fundPos[f.id];
              const currentNav = navs[f.id];
              const investedValue = pos ? pos.units * currentNav : 0;
              const navMove =
                f.nav > 0 ? ((currentNav - f.nav) / f.nav) * 100 : 0;

              return (
                <article
                  key={f.id}
                  data-hover="lift"
                  className={`${CARD} group relative flex w-[238px] shrink-0 snap-start flex-col overflow-hidden p-0 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-400/25 hover:shadow-[0_16px_40px_rgba(0,0,0,.35)] sm:w-[252px] lg:w-[268px]`}
                >
                  {/* Image */}
                  <div className="relative h-[92px] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#091118] via-[#091118]/35 to-black/10" />

                    <div className="absolute left-3 top-3 grid h-9 w-9 place-items-center rounded-xl border border-cyan-300/15 bg-[#071017]/75 text-cyan-300 backdrop-blur-md">
                      <f.icon className="h-4 w-4" />
                    </div>

                    <span
                      className={`absolute right-3 top-3 rounded-full px-2 py-1 text-[9px] font-semibold ring-1 ring-inset backdrop-blur-md ${RISK[f.risk]}`}
                    >
                      {f.risk}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-1 flex-col p-3.5">
                    ...
                    {/* NAV */}
                    <div className="mt-3 rounded-xl border border-white/[0.05] bg-white/[0.025] p-3">
                      ...
                      <div className="mt-2.5 h-1 overflow-hidden rounded-full bg-white/[0.05]">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-cyan-500/50 to-cyan-300 transition-all duration-700"
                          style={{
                            width: `${Math.min(
                              35 + Math.abs(f.growth) * 2,
                              92
                            )}%`,
                          }}
                        />
                    ...
                    <div className="mt-2.5 grid grid-cols-2 gap-2">
                      <div className="rounded-xl border border-white/[0.05] bg-white/[0.018] px-2.5 py-2">
                        <p className="text-[9px] text-slate-600">Your units</p>
                        <p className="mt-0.5 truncate text-[11px] font-semibold tabular-nums text-slate-200">
                          {pos ? fmt(pos.units, 2) : "None yet"}
                        </p>
                      </div>

                      <div className="rounded-xl border border-white/[0.05] bg-white/[0.018] px-2.5 py-2">
                        <p className="text-[9px] text-slate-600">Position</p>
                        <p className="mt-0.5 truncate text-[11px] font-semibold tabular-nums text-slate-200">
                          {pos ? fmt0(investedValue) : "—"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-2.5 flex items-center justify-between">
                      ...
                      <span
                        className={`text-[10px] ${
                          navMove >= 0 ? "text-emerald-300" : "text-rose-300"
                        }`}
                      >
                        {navMove >= 0 ? "+" : ""}
                        {navMove.toFixed(2)}%
                    ...
                    <button
                      ...
                      className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-cyan-400/20 bg-cyan-400/[0.06] py-2.5 text-xs font-semibold text-cyan-300 transition hover:border-cyan-300/40 hover:bg-cyan-400/[0.12]"
                    >
