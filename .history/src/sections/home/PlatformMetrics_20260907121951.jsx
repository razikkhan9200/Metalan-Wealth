      ...
      <Container>
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[1fr_1fr] lg:gap-16">
          <div>
            <Text
              size="sm"
              className="metrics-label !text-[12px] !font-semibold !tracking-[0.02em] !text-[#D4A853]"
            >
              PLATFORM METRICS
            </Text>
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {METRICS.map((metric) => (
                <Card
                  key={metric.label}
                  className="metric-card !min-h-[162px] !rounded-[16px] !border-[#D4A853]/30 !bg-[#121614]/[0.8784] !p-6 !shadow-none"
                >
                  <Text
                    size="sm"
                    className="!text-[11px] !font-medium !text-[#8F9A96]"
                  >
                    {metric.label}
                  </Text>

                  <div className="mt-3 font-mono text-[34px] font-bold leading-none text-[#D4A853]">
                    {metric.value}
                  </div>
            ...
            <div className="metrics-benefits mt-8 space-y-3">
              {BENEFITS.map((benefit) => (
                ...
                >
                  ...
                  <Text
                    size="sm"
                    className="!text-[14px] !leading-[21px] !text-[#D4A853]"
                  >
                    {benefit}
                  </Text>
                </div>
          ...
          <div className="metrics-visual relative mx-auto w-full max-w-[620px]">
            <div className="overflow-hidden rounded-[20px] border border-[#D4A853]">
              <img
            ...
            <Card className="absolute bottom-5 right-5 !w-[240px] !rounded-[16px] !border-white/15 !bg-[#101412]/95 !p-5 !shadow-none backdrop-blur-md sm:bottom-6 sm:right-6">
              ...
              <div className="mt-5 flex h-[52px] items-end gap-1.5">
                {[16, 20, 21, 30, 28, 38, 43, 48].map((height, index) => (
                  <span
                    key={index}
                    className={`flex-1 rounded-sm ${
                      index < 4 ? "bg-[#1A3C34]" : "bg-[#D4A853]"
                    }`}
